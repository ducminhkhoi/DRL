...
class Actor(nn.Module):
    def __init__(self, nb_states, nb_actions, action_type, use_expect, hidden1=20, hidden2=300, init_w=3e-3):
        ...
        self.fc1 = nn.Linear(6 * 6 * 64, hidden1)
...
class Critic(nn.Module):
    def __init__(self, nb_states, nb_actions, action_type, use_expect, hidden1=20, hidden2=300, init_w=3e-3):
        ...
        self.conv3 = nn.Conv2d(64, 64, kernel_size=3, stride=1)
        self.fc1 = nn.Linear(6 * 6 * 64, hidden1)
        self.fc2 = nn.Linear(hidden1+1, hidden2) if use_expect else nn.Linear(hidden1+2, hidden2)
...
class DDPG(Agent):
    ...
    def invert_gradient(self, d, g):
        if len(d.size()) == 1:
            for i in range(d.size(0)):
                if g[i] > 0:
                    g[i] *= (self.pmax - d[i]) / (self.pmax - self.pmin)
                else:
                    g[i] *= (d[i] - self.pmin) / (self.pmax - self.pmin)
        else:
    ...
    def update(self, state, action, reward, new_state, done):
        ...
        if len(self.experience_replay) >= self.observation:  # if have enough experience example, go
            ...
            #                 p.grad.data[j] *= (self.pmax - p.data[j])/(self.pmax - self.pmin)
            #             else:
            #                 p.grad.data[j] *= (p.data[j] - self.pmin)/(self.pmax - self.pmin)
